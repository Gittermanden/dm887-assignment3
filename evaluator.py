...
def run_experiment(env_id, algo_id, total_steps=100000, seed=42, n_envs=1, eval_freq=20000, buffer_size=100000):
    ...
    if "dm_control" in env_id:
        ...
        env = make_vec_env(_make_car_env, n_envs=n_envs, seed=seed, vec_env_cls=SubprocVecEnv)
        eval_env = make_vec_env(_make_car_env, n_envs=1, seed=seed)
    ...
    if algo_id == "PPO":
        ...
        model = SAC(policy_type, env, tensorboard_log=tb_log_dir, buffer_size=buffer_size, seed=seed)
    elif algo_id == "TD3":
        n_actions = env.action_space.shape[-1]
        action_noise = OrnsteinUhlenbeckActionNoise(mean=np.zeros(n_actions), sigma=0.1 * np.ones(n_actions))
        model = TD3(policy_type, env, tensorboard_log=tb_log_dir, buffer_size=buffer_size, action_noise=action_noise, seed=seed)
